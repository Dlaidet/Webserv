...
/*   Updated: 2023/05/04 11:44:58 by fleblanc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	empty = false;
	std::string error = errorPage.substr(0, errorPage.find('='));
...
void	Config::setLocations(std::ifstream &file, std::string buf)
{
	...
	if (_locations.empty())
	{
		tmp.insert(std::pair<std::string, std::string>("allow_methods", ""));
...
void	Config::setCgi(std::ifstream &file, std::string buf)
{
	...
	if (tmp.find("directory") == tmp.end())
		...
		tmp.insert(std::pair<std::string, std::string>("directory", tmp.find("directory")->second));
...
void	Config::checkClientBodyLimit()
{
	...
	if (std::atoi(_clientBodyLimit.c_str()) <= 0)
		throw(Config::ErrorBadArgument());
	if (std::atoi(_clientBodyLimit.c_str()) >= 65535)
		throw(Config::ErrorBadArgument());
}
...
void	Config::checkErrorPages()
{
	...
	for (int i = 0; i < 9; i++)
	{
		...
		{
			filepath = _root + "/" + _errorPages.find("error_page_" + errors[i])->second;
			if (filepath.find("//") < filepath.size())
				filepath.erase(filepath.find("//"), 1);
			file.open(filepath.c_str());
			if (file.fail())
				_errorPages.find("error_page_" + errors[i])->second.clear();
			file.close();
		}
	}
}
